...
def calculate_precision_at_k(recommendations, k=10, quality_threshold=3.5):
    ...
    Relevant = movies with rating >= quality_threshold
    """
    if len(recommendations) == 0:
        return 0.0
    
    relevant = (recommendations.head(k)['avg_rating'] >= quality_threshold).sum()
    precision = relevant / min(k, len(recommendations))
    return precision

def calculate_recall_at_k(recommendations, movies_df, k=10, quality_threshold=3.5):
    """
    ...
    total_relevant = (movies_df['avg_rating'] >= quality_threshold).sum()
    if total_relevant == 0:
        return 0.0
    
    relevant_in_k = (recommendations.head(k)['avg_rating'] >= quality_threshold).sum()
    recall = relevant_in_k / total_relevant
    return recall

def calculate_mean_average_precision(recommendations, k_values=[5, 10, 20]):
    """
    Calculate Mean Average Precision (MAP)
    Average precision across different k values
    """
    precisions = []
    for k in k_values:
        p = calculate_precision_at_k(recommendations, k)
        precisions.append(p)
...
def evaluate_recommendation_diversity(recommendations):
    ...
    High diversity = varied genres and years
    """
    if len(recommendations) == 0:
        return 0.0
    
    # Genre diversity
    genre_diversity = recommendations['genres'].nunique() / len(recommendations)
    
    # Year diversity
    year_diversity = recommendations['year'].nunique() / len(recommendations)
    
    # Combined score
    diversity_score = (genre_diversity + year_diversity) / 2
    return diversity_score

def evaluate_recommendation_similarity(recommendations):
    """
    Evaluate similarity strength of recommendations
...
def get_recommendation_metrics(recommendations, movies_df, method_name=""):
    """
    Get comprehensive metrics for recommendations
    """
    metrics = {
        'method': method_name,
        'num_recommendations': len(recommendations),
        'precision@5': calculate_precision_at_k(recommendations, k=5),
        'precision@10': calculate_precision_at_k(recommendations, k=10),
        'precision@20': calculate_precision_at_k(recommendations, k=20),
        'recall@5': calculate_recall_at_k(recommendations, movies_df, k=5),
        'recall@10': calculate_recall_at_k(recommendations, movies_df, k=10),
        'mean_avg_precision': calculate_mean_average_precision(recommendations),
        'diversity_score': evaluate_recommendation_diversity(recommendations),
        'avg_similarity': evaluate_recommendation_similarity(recommendations),
        'avg_rating': recommendations['avg_rating'].mean() if len(recommendations) > 0 else 0.0
    }
    
    return metrics

def print_metrics_report(metrics):
    """Pretty print recommendation metrics"""
    print(f"\n{'='*60}")
    print(f"Recommendation Metrics - {metrics.get('method', 'Unknown')}")
    print(f"{'='*60}")
    print(f"Total Recommendations: {metrics['num_recommendations']}")
    print(f"\nPrecision Scores:")
    print(f"  Precision@5:  {metrics['precision@5']:.1%}")
    print(f"  Precision@10: {metrics['precision@10']:.1%}")
    print(f"  Precision@20: {metrics['precision@20']:.1%}")
    print(f"\nRecall Scores:")
    print(f"  Recall@5:  {metrics['recall@5']:.1%}")
    print(f"  Recall@10: {metrics['recall@10']:.1%}")
    print(f"\nOther Metrics:")
    print(f"  Mean Avg Precision: {metrics['mean_avg_precision']:.1%}")
